...
public class MessageReceiverWithoutJNDI117 implements ExceptionListener,
	...
	private static final String ALAEM_TOPIC = "monitor_topic";
	private static final String DPL_TOPIC = "monitor_topic";
	...
	private void init() throws JMSException, NamingException,
        ...
        connectionParams.put(TransportConstants.HOST_PROP_NAME, "10.180.36.57");
        ...
        MyTransportConfiguration transportConfiguration = new MyTransportConfiguration(NettyConnectorFactory.class.getName(),
                connectionParams);
        ...
        Destination queue = null;
		 try {
	        	queue = HornetQJMSClient.createTopic(ALAEM_TOPIC);
	        	messageConsumer = session.createConsumer(queue); 
			} catch (Exception e) {
				queue = HornetQJMSClient.createQueue(ALAEM_TOPIC);
				messageConsumer = session.createConsumer(queue); 
			}
		messageConsumer = session.createConsumer(queue, "");
