...
public class MessageReceiverWithoutJNDI117 implements ExceptionListener,
	...
	private void init() throws JMSException, NamingException,
    	...
    	connectionParams.put(TransportConstants.PORT_PROP_NAME, 15353);
        connectionParams.put(TransportConstants.HOST_PROP_NAME, "10.110.2.127");
        connectionParams.put(TransportConstants.SSL_ENABLED_PROP_NAME, "true");
        connectionParams.put(TransportConstants.KEYSTORE_PATH_PROP_NAME,"/opt/netwatcher/pm4h2/work/conf/security/jms/EAMJMSKeystore.jks");
        connectionParams.put(TransportConstants.KEYSTORE_PASSWORD_PROP_NAME,"ACROSS_eamjms_cer_2016");
        connectionParams.put(TransportConstants.TRUSTSTORE_PATH_PROP_NAME,"/opt/netwatcher/pm4h2/work/conf/security/jms/EAMJMSKeystore.truststore");
        connectionParams.put(TransportConstants.TRUSTSTORE_PASSWORD_PROP_NAME,"ACROSS_eamjms_cer_2016");
        ...
        connections = cf.createConnection("jmseam","ACROSS_eam_2013");    
		connections.setExceptionListener(this);

		session = connections.createSession(false, Session.AUTO_ACKNOWLEDGE);
        Destination queue = null;
		 try {
	        	queue = HornetQJMSClient.createTopic("topic_eam");
	        	messageConsumer = session.createConsumer(queue); 
			} catch (Exception e) {
				queue = HornetQJMSClient.createQueue("topic_eam");
				messageConsumer = session.createConsumer(queue); 
			}
		messageConsumer = session.createConsumer(queue, "");
