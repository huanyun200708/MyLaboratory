...
public class HandleJavaFormateFileImp implements HandleJavaFormateFile {
	...
	public boolean isNeededLine(String line) {
		return (line.matches(".*\\.info\\(.*")
				||line.matches(".*\\.debug\\(.*")
				||line.matches(".*\\.error\\(.*"))
				&& (line.toLowerCase().contains("pass")||line.toLowerCase().contains("pwd"))
				;
	}
